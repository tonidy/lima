...
func stopInstanceGracefully(inst *store.Instance) error {
	...
	return waitForHostAgentAbortion(context.TODO(), inst)
}

func waitForHostAgentAbortion(ctx context.Context, inst *store.Instance) error {
	ctx2, cancel := context.WithTimeout(ctx, 3*time.Minute)
	defer cancel()

	var receivedAbortedEvent bool
	onEvent := func(ev hostagentapi.Event) bool {
		if len(ev.Status.Errors) > 0 {
			logrus.Errorf("%+v", ev.Status.Errors)
		}
		if ev.Status.Aborted {
			receivedAbortedEvent = true
			return true
		}
		return false
	}
	...
	if !receivedAbortedEvent {
		return errors.New("did not receive an event with the \"aborted\" (i.e., poweroff) status")
	}
